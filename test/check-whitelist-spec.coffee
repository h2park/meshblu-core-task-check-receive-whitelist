...
describe 'CheckWhitelist', ->
  ...
  describe '->do', ->
    describe 'when called with a valid job', ->
      beforeEach (done) ->
        @whitelistManager.canReceive.yields null, true
        job =
          metadata:
            auth:
              uuid: 'green-blue'
    ...
    describe 'when called with a valid job without a fromUuid', ->
      ...
      it 'should call the whitelistmanager with the correct arguments', ->
        expect(@whitelistManager.canReceive).to.have.been.calledWith fromUuid: 'green-blue', toUuid: 'bright-green'
    ...
    describe 'when called with a job that with a device that has an invalid whitelist', ->
      beforeEach (done) ->
        @whitelistManager.canReceive.yields null, false
        job =
          metadata:
            auth:
              uuid: 'puke-green'
    ...
    describe 'when called and the canReceive yields an error', ->
      beforeEach (done) ->
        @whitelistManager.canReceive.yields new Error "black-n-black"
        job =
          metadata:
            auth:
              uuid: 'puke-green'
